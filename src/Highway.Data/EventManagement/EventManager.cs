    ...
    where T : class
{
    ...
    private void HandleEvent(object sender, AfterSaveEventArgs e)
    {
        var events = _interceptors.OfType<IEventInterceptor<AfterSaveEventArgs>>().OrderBy(x => x.Priority);
    ...
    private void HandleEvent(object sender, BeforeSaveEventArgs e)
    {
        var events = _interceptors.OfType<IEventInterceptor<BeforeSaveEventArgs>>().OrderBy(x => x.Priority);
        foreach (var eventInterceptor in events)
        {
            var result = eventInterceptor.Apply(_repository.DomainContext, e);
            if (!result.ContinueExecution)
