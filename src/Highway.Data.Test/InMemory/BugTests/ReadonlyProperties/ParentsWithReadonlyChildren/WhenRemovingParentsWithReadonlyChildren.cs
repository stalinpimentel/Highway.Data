...
public class WhenRemovingParentsWithReadonlyChildren
{
    ...
    public void TheCorrectExceptionShouldBeThrown()
    {
        ...
        removeParent
            ...
            .WithMessage(
                $"An entry could not be removed from the {nameof(InMemoryDataContext)} because its referencing property has no setter. The entry"
                + $" was scheduled for removal because it is referenced from {typeof(ParentBase).FullName} through the property"
                + $" {nameof(ParentBase)}.{nameof(_parent1.Children)}. Either add a setter to this property, or decorate it with the"
